...
class employeeClass:
    def __init__(self,root):
        ...
        cmb_utype=ttk.Combobox(self.root,textvariable=self.var_utype,values=("Employee"),state=DISABLED,justify=CENTER,font=("goudy old style",15))
...
#-------------------------------------
    def add(self):
        # self.number=re.search(self.pattern,self.var_contact.get())
        # self.email=re.search(self.email_condition,self.var_email.get())
        ...
        try:
            if self.var_emp_id.get()=="":
                ...
                if row==None:
                    ...
                    cur.execute("select utype from employee where eid=?",(self.var_emp_id.get(),))
                    row1=cur.fetchone()
                    if row1[0]=='Admin':
                        messagebox.showerror("Error","You cannot delete admins record")
                    else:
